---
name: image-to-code
description: Elite website image-to-code skill. For visually important web tasks, it first
  generates the design image(s) via the Kie AI `generate` skill, deeply analyzes them, then
  implements the website to match them as closely as possible. Prefers large, readable,
  section-specific images over tiny compressed boards, generates fresh standalone images for
  sections or detail views instead of cropping old ones, avoids lazy under-generation, avoids
  cards-inside-cards-inside-cards UI, and keeps the hero clean, spacious, readable, and visible
  on a small laptop. Every image batch is balance-checked, cost-quoted, and approved before it
  runs. Trigger on requests to design and build a website, landing page, or marketing site where
